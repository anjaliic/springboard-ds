...
http://frankfletcher.co/springboard_phpmyadmin
...
SELECT DISTINCT name AS Court, CONCAT(firstname,' ', surname) AS fullname
FROM Facilities
INNER JOIN Bookings
USING(facid)
INNER JOIN Members
...
WHERE starttime LIKE '2012-09-14%' 
    AND ((memid > 0 AND (membercost * slots) >= 30) OR (memid = 0 AND (guestcost * slots) >= 30))
ORDER BY cost DESC;
...
SELECT name AS Facility, CONCAT(surname, ' ', firstname) AS Member, SUM(slots) AS Total_Usage
FROM Bookings
LEFT JOIN Facilities
USING(facid)
LEFT JOIN Members
...
WHERE memid > 0
GROUP BY member
ORDER BY name, Total_Usage DESC;
...
USING(memid)
WHERE memid > 0
GROUP BY name, strftime('%Y-%m', starttime)
ORDER BY strftime('%Y-%m', starttime), name;